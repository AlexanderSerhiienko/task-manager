...
export function WorkspaceShellSkeleton() {
  return (
    <main className="mx-auto flex w-full max-w-5xl flex-col gap-5 py-4">
      ...
      <section className="rounded-2xl border border-zinc-800 bg-zinc-950/75 p-5 shadow-xl">
...
export function ProjectListSkeleton({ count = 4 }: ListSkeletonProps) {
  return (
    <ul className="space-y-3">
      {Array.from({ length: count }).map((_, index) => (
        <li key={`project-skeleton-${index}`} className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-4">
          <div className="flex items-center justify-between gap-3">
...
export function MemberListSkeleton({ count = 4 }: ListSkeletonProps) {
  return (
    <ul className="space-y-3">
      {Array.from({ length: count }).map((_, index) => (
        <li key={`member-skeleton-${index}`} className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-4">
          <div className="flex items-center justify-between gap-3">
            <div className="w-full max-w-sm space-y-2">
              <SkeletonLine className="h-4 w-2/3" />
              <SkeletonLine className="h-3 w-1/2" />
    ...
    <div className="space-y-4">
      <div className="space-y-4">
        {Array.from({ length: 3 }).map((_, sectionIndex) => (
          <div key={`task-section-skeleton-${sectionIndex}`} className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4">
            <SkeletonLine className="mb-3 h-4 w-32" />
            <div className="space-y-3">
              {Array.from({ length: 2 }).map((__, cardIndex) => (
                <div key={`task-card-skeleton-${sectionIndex}-${cardIndex}`} className="rounded-xl border border-zinc-700/80 bg-zinc-950/70 p-4">
    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {Array.from({ length: 4 }).map((_, index) => (
          <div key={`overview-card-skeleton-${index}`} className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-4">
            <div className="space-y-2">
              <SkeletonLine className="h-3 w-20" />
              <SkeletonLine className="h-8 w-16" />
            </div>
